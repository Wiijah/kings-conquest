...
function spawnUnit(data, isCreation, row, column){
    ...
    if (data.address == "graphics/spritesheet/stand/ss_rogue_stand.png"|| data.address == "graphics/spritesheet/stand/ss_scarecrow_stand.png") return;
        ...
        unit.commandable = data.commandable == 1 ? true : false;
		unit.hp = data.hp;
...
function applyBuff(buffType, unit) {
	...
	switch (buffType) {
        ...
        case 5: // burn Buff
            ...
            unit.buffs.push([5, 0.03, 6, buffIcon]);
            break;
        case 6: // forzen Buff
...
function drawUnitCreationMenu() {
	...
	createFloatingCards(listOfSources, ["knight","archer","wizard","dragon","totem"]);
...
function findAvailableAndNonAvailableSpawnTiles(range) {
    ...
    var originX = turn == 0 ? 0 : 12;
    var originY = turn == 0 ? 0 : 13;
    var di = turn == 0 ? 1 : -1;
    var dj = turn == 0 ? 1 : -1;
...
function addEventListenersToUnit(unit) {
    ...
    unit.addEventListener("click", function(event) {
            ...
            if (!movingPlayer && !isAttacking && !isCasting) {
                ...
                if (unit.team == turn && unit.commandable) showActionMenuNextToPlayer(unit);
                displayStats(unit);
            }
            ...
            if (selectedCharacter != unit && isCasting && selectedCharacter.team != unit.team && selectedCharacter.skill_no == 4) {
            ...
            // In this case, we are selecting the unit to be attacked by the dragon spell
...
function showActionMenuNextToPlayer(unit) {
	...
	console.log("skillCoolDown: " + unit.skillCoolDown + "," + "outOfMoves: " + unit.outOfMoves);
